...
class RAGASEvaluator:
    ...
    def evaluate_context_recall(self, contexts: list[str], expected: str) -> float:
        ...
        if not expected_tokens:
            return 1.0
        retrieved_tokens = set().union(*(_tokenize(chunk) for chunk in contexts)) if contexts else set()
...
class LLMJudge:
    ...
    def score_response(
        ...
        prompt = (
            "Score the response against every rubric criterion. Return a JSON object "
            "whose keys are the criterion names and whose values are scores from 0 to 1.\n\n"
            f"Question:\n{question}\n\nResponse:\n{answer}\n\nRubric:\n{rubric_text}"
        )
        ...
        try:
            ...
            scores = {
                criterion: min(1.0, max(0.0, float(parsed[criterion])))
                for criterion in rubric
                if criterion in parsed and isinstance(parsed[criterion], (int, float))
            }
    ...
    def detect_bias(self, scores_batch: list[dict[str, Any]]) -> dict[str, Any]:
        ...
        for index, result in enumerate(scores_batch):
            ...
            if values:
                ...
                (first_scores if index % 2 == 0 else later_scores).append(average)
        overall_average = sum(averages) / len(averages) if averages else 0.0
        positional = bool(first_scores and later_scores and (sum(first_scores) / len(first_scores) > sum(later_scores) / len(later_scores) + 0.1))
        return {
...
class FailureAnalyzer:
    ...
    def generate_improvement_log(self, failures: list, suggestions: list[str]) -> str:
        ...
        for index, failure in enumerate(failures, start=1):
            ...
            suggestion = suggestions[index - 1] if index <= len(suggestions) else "Review this failure cluster"
            root_cause = self.find_root_cause(failure)
            safe = lambda value: str(value).replace("|", "\\|")
            rows.append(
                f"| {safe(failure_id)} | {safe(failure.failure_type or 'unknown')} | "
                f"{safe(root_cause)} | {safe(suggestion)} | Open |"
            )
        return "\n".join(rows)
    ...
    def generate_improvement_suggestions(
        ...
        mapping = {
            "hallucination": "Add a grounding check that removes unsupported claims and prompts the assistant to state uncertainty.",
            "irrelevant": "Add intent-focused prompt instructions and representative question-to-answer examples.",
            "incomplete": "Retrieve more targeted evidence and require the generator to cover dates, conditions, and exceptions.",
            "off_topic": "Add intent routing and a pre-answer relevance check before generation.",
            "refusal": "Review guardrail boundaries and add safe-answer examples for in-scope questions.",
        }
        suggestions = [mapping[kind] for kind, _ in sorted(counts.items(), key=lambda item: (-item[1], item[0])) if kind in mapping]
        generic = [
            "Add the failed cases to the regression dataset and gate releases on the three answer-side metrics.",
            "Review retrieved chunks for each failed case and tune chunking or ranking before changing the prompt.",
            "Re-run the benchmark after every retrieval or prompt change and compare against the saved baseline.",
        ]
